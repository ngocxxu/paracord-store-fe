...
export const CATEGORIES: ShopCategory[] = ['bracelets', 'keychains'];
...
export function parseShopParams(
  ...
  return {
    ...
    weaveType: (WEAVE_TYPES.includes(get('weaveType') as WeaveType)
      ? get('weaveType')
      : null) as WeaveType | null,
    ...
    sort: SORT_OPTIONS.includes(get('sort') as SortOption)
...
export function filterProducts(
  ...
  return products.filter((p) => {
    if (params.categories.length && !params.categories.includes(p.category))
      return false;
    ...
    if (p.priceNum < params.minPrice || p.priceNum > params.maxPrice)
      return false;
    return true;
  });
}
...
export function buildShopQuery(
  partial: Partial<ShopParams>,
  current: ShopParams,
): string {
  const q = new URLSearchParams();
  const categories =
    partial.categories !== undefined ? partial.categories : current.categories;
  const weaveType =
    partial.weaveType !== undefined ? partial.weaveType : current.weaveType;
  const colors = partial.colors !== undefined ? partial.colors : current.colors;
  const minPrice =
    partial.minPrice !== undefined ? partial.minPrice : current.minPrice;
  const maxPrice =
    partial.maxPrice !== undefined ? partial.maxPrice : current.maxPrice;
...
export function getCategoryCounts(
  products: ResolvedShopProduct[],
): Record<ShopCategory, number> {
  const counts = { bracelets: 0, lanyards: 0, keychains: 0 } as Record<
    ShopCategory,
    number
  >;
